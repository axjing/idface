...
from .common import cvtColor, img_normalization, img_resize
...
class FacenetDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        image = img_resize(image, [self.input_shape[1], self.input_shape[0]], letterbox_image = True)
        image = img_normalization(np.array(image, dtype='float32'))
...
class LFWDataset(datasets.ImageFolder):
    ...
    def __getitem__(self, index):
        ...
        image1 = img_resize(image1, [self.image_size[1], self.image_size[0]], letterbox_image = True)
        image2 = img_resize(image2, [self.image_size[1], self.image_size[0]], letterbox_image = True)
        
        image1, image2 = np.transpose(img_normalization(np.array(image1, np.float32)),[2, 0, 1]), np.transpose(img_normalization(np.array(image2, np.float32)),[2, 0, 1])
